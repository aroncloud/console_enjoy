<template>
  <div class="bg-white rounded-2xl p-5 flex items-center justify-between border border-gray-100">

    <!-- Skeleton -->
    <template v-if="loading">
      <div class="flex flex-col gap-3 animate-pulse w-full">
        <div class="h-3 w-24 bg-gray-200 rounded" />
        <div class="h-8 w-32 bg-gray-200 rounded" />
        <div class="h-3 w-36 bg-gray-100 rounded" />
      </div>
      <div class="w-12 h-12 bg-gray-200 rounded-xl shrink-0 animate-pulse" />
    </template>
